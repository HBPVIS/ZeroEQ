...
struct Frame
{
    ...
    bool operator == ( const zeroeq::hbp::data::Frame& rhs ) const
    {
 ...
 * to avoid copy. The data pointer is only valid as long as the ZeroEQ Event
 ...
 */
ZEROEQ_API Event serializeCamera( const std::vector< float >& matrix );
 ...
 */
ZEROEQ_API std::vector< float > deserializeCamera( const Event& event );
 ...
 * @return the serialized event.
 */
ZEROEQ_API Event serializeLookupTable1D( const std::vector< uint8_t >& lut );
 ...
 * @return the serialized event.
 */
ZEROEQ_API Event serializeImageJPEG( const data::ImageJPEG& image );

/**
 * Deserialize the given EVENT_IMAGEJPEG event into an JPEG image.
 * @param event the ZeroEQ EVENT_IMAGEJPEG.
 * @return the jpeg image.
 */
ZEROEQ_API data::ImageJPEG deserializeImageJPEG( const Event& event );
