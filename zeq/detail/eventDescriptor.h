...
{
    EventDescriptor( const std::string& restName, const lunchbox::uint128_t& eventType,
    ...
    const lunchbox::uint128_t& getEventType() const { return _eventType; }
    const std::string& getSchema() const { return _schema; }

private:
    const std::string _restName;
    const lunchbox::uint128_t _eventType;
